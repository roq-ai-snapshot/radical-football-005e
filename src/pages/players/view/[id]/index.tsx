...
import { deleteCoachPlayerById } from 'apiSdk/coach-players';
...
function PlayerViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PlayerInterface>(
    ...
    () =>
      getPlayerById(id, {
        relations: ['user', 'academy', 'coach_player'],
      }),
  );

  const coach_playerHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('coach_player', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Coach Players
                    </Text>
                    <NextLink passHref href={`/coach-players/create?player_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.coach_player?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/coach-players/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('coach_player', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink passHref href={`/coach-players/edit/${record.id}`}>
                              ...
                              {hasAccess('coach_player', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={() => coach_playerHandleDelete(record.id)}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
                                  icon={<FiTrash />}
